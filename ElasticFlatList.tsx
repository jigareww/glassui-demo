...
export interface ElasticFlatListProps<T> extends FlatListProps<T> {
  horizontal?: boolean;
  elastic?: boolean; // Toggles the custom bounce physics
}

/**
 * Custom hook to maintain a stable callback reference.
 * Passing inline functions to FlatList causes severe re-render performance drops.
 * This guarantees the internal callbacks never change their reference.
 */
...
function ElasticFlatListInner<T>(
  ...
  if (Platform.OS === 'ios' || !elastic) {
    return (
      <FlatList
        ...
        bounces={elastic !== false} // On iOS, disable bounces if elastic is explicitly false
        style={style}
  ...
  // Primitive shared values avoid object allocation overhead in worklets (Performance Optimization)
  ...
  const pan = Gesture.Pan()
    ...
    .onTouchesMove((e, stateManager) => {
      ...
      if (e.allTouches.length > 0) {
        ...
        // Wait for 10px of movement before aggressively claiming the gesture
        if (Math.abs(delta) > 10) {
          const dir = edgeDirection(delta);
          
          if (dir !== 0 || isOverscrolling.value) {
            // Outward edge pull detected -> Claim the touch!
            stateManager.activate();
          } else {
            // Normal scrolling -> Abort custom gesture instantly, yield 100% to Native FlatList!
            stateManager.fail();
          }
        }
      }
    ...
    .onUpdate((e) => {
      ...
      // Inline Apple rubber band physics equation
    ...
    <GestureDetector gesture={pan}>
      <Animated.View style={[style, animatedStyle]}>
        <FlatList
          ref={ref}
          horizontal={horizontal}
          overScrollMode="never" // Disable Android native glow effect
          bounces={false}
          contentContainerStyle={contentContainerStyle}
          onScroll={onScroll}
...
const ElasticFlatList = forwardRef(ElasticFlatListInner) as <T>(
  props: ElasticFlatListProps<T> & { ref?: React.ForwardedRef<FlatList<T>> }
) => React.ReactElement;

export default ElasticFlatList;
