...
import { RENDER_MODES } from "../../shared/ui/Button/Button.constants";
import { getGlassRenderMode } from "../../shared/ui/Button/Button.utils";
...
export const Card: React.FC<CardProps> = memo(
  ({
    ...
    React.useEffect(() => {
      const configHash = `card-${renderMode}-${audit.isSupported}-${
      ...
      if (renderMode !== RENDER_MODES.LIQUID_GLASS && audit.isIos) {
        console.warn(
          `[Card] Liquid Glass card disabled. Fallback Reason: ${
      ...
      return (
        ...
        >
          <NativeLiquidGlassView
            ...
            tintColor={
              isDarkMode ? "rgba(8, 17, 44, 0.65)" : "rgba(116, 116, 128, 0.08)"
            }
            pointerEvents="none"
          />
          {effect === "none" && (
      ...
      <View
        style={[
          ...
          isDarkMode ? styles.solidDark : styles.solidLight,
          {
            borderRadius: activeBorderRadius,
            borderWidth: 1,
            borderColor: isDarkMode ? "rgba(255, 255, 255, 0.1)" : "#E5E5EA",
          },
        ]}
        pointerEvents="none"
      />
    ...
    return (
      <View style={containerStyle} renderToHardwareTextureAndroid={false}>
        {renderLayers()}
